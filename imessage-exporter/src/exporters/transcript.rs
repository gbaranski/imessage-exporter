...
impl<'a> Exporter<'a> for Transcript<'a> {
    ...
    fn iter_messages(&mut self) -> Result<(), RuntimeError> {
        ...
        for message in messages {
            ...
            //     Transcript::write_to_file(self.get_or_create_file(&msg), &announcement);
            // }
            ...
            if !msg.is_reaction() {
                ...
                let message = self
                    ...
                    .map_err(RuntimeError::DatabaseError)?;
                Transcript::write_to_file(self.get_or_create_file(&msg), &message);
            }
            current_message += 1;
...
impl<'a> Writer<'a> for Transcript<'a> {
    fn format_message(&self, message: &Message, indent_size: usize) -> Result<String, TableError> {
        ...
        let message_parts = body.iter().filter_map(|part| match part {
            BubbleType::Text(text) => Some(text.to_string()),
            BubbleType::Attachment => None,
            BubbleType::App => None,
        });
        for (idx, message_part) in message_parts.enumerate() {
            self.add_line(&mut formatted_message, &format!("{}: {}", who, message_part), &indent);
        }
        return Ok(formatted_message);
...
impl<'a> BalloonFormatter<&'a str> for Transcript<'a> {
...
mod tests {
    ...
    fn can_create() {
        ...
        let exporter = Transcript::new(&config);
...
mod balloon_format_tests {
    ...
    use crate::{exporters::exporter::BalloonFormatter, Config, Exporter, Transcript};
